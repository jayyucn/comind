...
pub async fn execute_batch(
    ...
    // 分派单源在 core（ADR-0048 / #116）：本命令只剩事务包裹 + page touch +
    // sync 通知。op 语义变更只改 comind-core services/batch.rs 一处。
    let result = execute_with_transaction_adapter(db, |storage| {
        let effects = comind_core::services::batch::apply_batch(storage, &operations)?;

        // page touch：保持原语义在事务内（随批回滚）
        let mut page_ids: std::collections::HashSet<String> = std::collections::HashSet::new();
        for e in &effects {
            page_ids.extend(e.page_ids.iter().cloned());
        }
        ...
        for e in &effects {
            for (table, id) in &e.sync {
                sync_changes.entry(*table).or_default().push(id.clone());
            }
        }

        let values = effects.into_iter().map(|e| e.value).collect();
        Ok((values, sync_changes))
    })
    .await;
...
#[tauri::command]
pub async fn apply_relationship_type_to_block_content(
    content: String,
    target_title: String,
...
/// Returns `{ has_typed_link: bool }`.
#[tauri::command]
pub async fn check_has_typed_link_to_target(
    content: String,
    target_title: String,
) -> Result<serde_json::Value, String> {
...
/// Used by backlinks to sort references by block position.
#[tauri::command]
pub async fn build_document_order(
    db: State<'_, super::state::DatabaseConnection>,
    page_id: String,
