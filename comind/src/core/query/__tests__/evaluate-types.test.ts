...
describe('number', () => {
  it('eq / neq', () => {
    expect(matchCondition({ field: 'score', op: 'eq', value: 10 }, recs[0], reg, 'rec')).toBe(true)
    expect(matchCondition({ field: 'score', op: 'neq', value: 10 }, recs[1], reg, 'rec')).toBe(true)
    expect(matchCondition({ field: 'score', op: 'eq', value: 10 }, recs[1], reg, 'rec')).toBe(false)
  })
  it('gt / lt', () => {
    const q = query(grp('or', [{ field: 'score', op: 'gt', value: 15 }, { field: 'score', op: 'lt', value: 15 }]))
    const out = evaluate(q, recs, reg, 'rec')
    expect(out.map((r) => r.score).sort((a, b) => (a ?? 0) - (b ?? 0))).toEqual([10, 20]) // 15 不满足任一
  })
  it('空值：比较遇空即 false', () => {
    expect(matchCondition({ field: 'score', op: 'eq', value: 10 }, recs[2], reg, 'rec')).toBe(false)
...
describe('date（日粒度，yyyy-MM-dd）', () => {
  it('before / after', () => {
    expect(matchCondition({ field: 'due', op: 'before', value: '2026-02-01' }, recs[0], reg, 'rec')).toBe(true)
    expect(matchCondition({ field: 'due', op: 'before', value: '2026-02-01' }, recs[3], reg, 'rec')).toBe(false)
    const q = query(grp('or', [{ field: 'due', op: 'after', value: '2026-01-31' }]))
    expect(evaluate(q, recs, reg, 'rec').map((r) => r.due).sort()).toEqual(['2026-02-10', '2026-03-01'])
  })
  it('between 含端点（日粒度）', () => {
    const q = query(grp('and', [{ field: 'due', op: 'between', value: ['2026-01-01', '2026-02-28'] }]))
    expect(evaluate(q, recs, reg, 'rec').map((r) => r.due).sort()).toEqual(['2026-01-15', '2026-02-10'])
  })
  it('空值：比较遇空即 false', () => {
    expect(matchCondition({ field: 'due', op: 'after', value: '2026-01-01' }, recs[2], reg, 'rec')).toBe(false)
  })
})

describe('multiSelect', () => {
  it('hasAny：满足任一', () => {
    const q = query(grp('and', [{ field: 'tags', op: 'hasAny', value: ['b'] }]))
  ...
  it('hasAll：满足全部', () => {
    const q = query(grp('and', [{ field: 'tags', op: 'hasAll', value: ['a', 'b'] }]))
    expect(evaluate(q, recs, reg, 'rec').map((r) => r.tags)).toEqual([['a', 'b']])
  })
  it('删除选项降级：引用的 id 不在选项集合 → 非匹配', () => {
    const q = query(grp('and', [{ field: 'tags', op: 'hasAny', value: ['x'] }]))
...
describe('boolean', () => {
  it('is true / is false', () => {
    expect(matchCondition({ field: 'archived', op: 'is', value: true }, recs[1], reg, 'rec')).toBe(true)
    expect(matchCondition({ field: 'archived', op: 'is', value: false }, recs[0], reg, 'rec')).toBe(true)
    const q = query(grp('and', [{ field: 'archived', op: 'is', value: true }]))
    expect(evaluate(q, recs, reg, 'rec').map((r) => r.archived)).toEqual([true])
  })
  it('空值：is 遇 null 即 false', () => {
    expect(matchCondition({ field: 'archived', op: 'is', value: false }, recs[2], reg, 'rec')).toBe(false)
  })
})
