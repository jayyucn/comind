...
describe('evaluate 入口', () => {
  it('返回过滤后的新集合，不修改入参', () => {
    ...
    const q = query(group('and', [{ field: 'status', op: 'is', value: 'open' }]))
...
describe('text 操作符', () => {
  const reg = makeRegistry()
  it('is 精确匹配（区分大小写）', () => {
    expect(matchCondition({ field: 'name', op: 'is', value: 'Gamma' }, rows[2], reg, 'row')).toBe(true)
    expect(matchCondition({ field: 'name', op: 'is', value: 'gamma' }, rows[2], reg, 'row')).toBe(false)
  })
  it('isNot', () => {
    expect(matchCondition({ field: 'name', op: 'isNot', value: 'Gamma' }, rows[0], reg, 'row')).toBe(true)
  })
  it('contains 不区分大小写', () => {
    expect(matchCondition({ field: 'name', op: 'contains', value: 'ALPHA' }, rows[3], reg, 'row')).toBe(true)
  })
  it('notContains', () => {
    expect(matchCondition({ field: 'name', op: 'notContains', value: 'task' }, rows[1], reg, 'row')).toBe(true)
  })
...
describe('select 操作符（按 id 比对）', () => {
  ...
  it('is / isNot 按选项 id 比对', () => {
    expect(matchCondition({ field: 'status', op: 'is', value: 'done' }, rows[1], reg, 'row')).toBe(true)
    expect(matchCondition({ field: 'status', op: 'isNot', value: 'done' }, rows[0], reg, 'row')).toBe(true)
  })
  it('引用的选项 id 已从字段删除时降级为非匹配（不抛错）', () => {
    const reg2 = makeRegistry([{ id: 'open', label: '进行中' }]) // done 被删
    expect(matchCondition({ field: 'status', op: 'is', value: 'done' }, rows[1], reg2, 'row')).toBe(false)
    expect(matchCondition({ field: 'status', op: 'isNot', value: 'done' }, rows[1], reg2, 'row')).toBe(false)
  })
})
...
describe('空值通行语义', () => {
  const reg = makeRegistry()
  it('比较类操作符遇空返回 false', () => {
    expect(matchCondition({ field: 'status', op: 'is', value: 'open' }, rows[2], reg, 'row')).toBe(false)
    expect(matchCondition({ field: 'status', op: 'isNot', value: 'open' }, rows[2], reg, 'row')).toBe(false)
  })
...
describe('防御性', () => {
  it('字段未在注册表时条件返回 false，不抛错', () => {
    const reg = makeRegistry()
    expect(matchCondition({ field: 'ghost', op: 'is', value: 'x' }, rows[0], reg, 'row')).toBe(false)
    expect(evalGroup(group('and', [{ field: 'ghost', op: 'is', value: 'x' }]), rows[0], reg, 'row')).toBe(false)
  })
})
