 ...
 * @param childrenLength - 可选：子节点数量，用于监听变化自动重建
 * @returns Sortable 实例的 ref（包含 reset 方法）
 */
export function useSortable(containerRef: Ref<HTMLElement | null>, childrenLength?: Ref<number>) {
  ...
  // 监听子节点数量变化，自动重建 Sortable 实例
  // 解决缩进/反缩进后 DOM 与 Sortable 状态不同步的问题
  if (childrenLength) {
    watch(childrenLength, () => {
