...
describe('useContentRenderer — 渲染测试', () => {
  ...
  describe('WikiLink 渲染', () => {
    it('内部链接 [[页面名]]', () => {
      const result = renderContentToHtml({ segments: [], content: '这是 [[项目A]] 的笔记' })
      expect(result).toContain('data-page="项目A"')
      expect(result).toContain('项目A')
    })

    it('带别名的链接 [[页面名|显示名]]', () => {
      const result = renderContentToHtml({ segments: [], content: '参考 [[项目A|A项目]]' })
      expect(result).toContain('data-page="项目A"')
      expect(result).toContain('A项目')
    })

    it('外部链接 [[https://...]]', () => {
      const result = renderContentToHtml({ segments: [], content: '访问 [[https://example.com]]' })
  ...
  describe('#tag 渲染为 Page 链接', () => {
    it('基础标签 #标签', () => {
      const result = renderContentToHtml({ segments: [], content: '这是 #标签' })
      expect(result).toContain('block-tag')
      expect(result).toContain('data-page="标签"')
    })

    it('中文标签 #中文', () => {
      const result = renderContentToHtml({ segments: [], content: '#中文 #日本語' })
      expect(result).toContain('data-page="中文"')
      expect(result).toContain('data-page="日本語"')
    })

    it('带层级的标签 #项目/子项目', () => {
      const result = renderContentToHtml({ segments: [], content: '#项目/子项目' })
      expect(result).toContain('data-page="项目/子项目"')
    })

    it('含点的标签部分匹配 #v1.0', () => {
      const result = renderContentToHtml({ segments: [], content: '#v1.0' })
      expect(result).toContain('block-tag')
      expect(result).toContain('#v1')
    })

    it('排除 URL 上下文 #', () => {
      const result = renderContentToHtml({ segments: [], content: 'https://example.com/#section' })
      expect(result).not.toContain('block-tag')
    })

    it('排除数字开头的 #123', () => {
      const result = renderContentToHtml({ segments: [], content: '#123' })
      expect(result).not.toContain('block-tag')
    })
  })
  ...
  describe('混合场景', () => {
    it('同时包含 WikiLink 和 #tag', () => {
      const result = renderContentToHtml({ segments: [], content: '查看 [[项目A]] 和 #标签' })
      expect(result).toContain('data-page="项目A"')
      expect(result).toContain('data-page="标签"')
    })

    it('纯文本', () => {
      const result = renderContentToHtml({ segments: [], content: '这是纯文本' })
...
describe('useContentRenderer - typed wiki links', () => {
  ...
  it('((type))[[X]] 渲染为关系标签 + block-link，关系标签显示中文label', () => {
    const html = renderContentToHtml({ segments: [], content: 'See ((depends-on))[[X]] for details', blockId: 'block-1' })
  ...
  it('关系标签的显示文本 = 中文label', () => {
    expect(renderContentToHtml({ segments: [], content: '((depends-on))[[A]]', blockId: 'b' })).toMatch(
      /<span class="rel-type-label"[^>]*>依赖<\/span>/
    )
    expect(renderContentToHtml({ segments: [], content: '((required-by))[[A]]', blockId: 'b' })).toMatch(
      /<span class="rel-type-label"[^>]*>被依赖<\/span>/
    )
    expect(renderContentToHtml({ segments: [], content: '((is-a))[[A]]', blockId: 'b' })).toMatch(
      /<span class="rel-type-label"[^>]*>是一个<\/span>/
    )
  })

  it('未知类型显示 unknown-type（fallback 到类型本身）', () => {
    const html = renderContentToHtml({ segments: [], content: '((unknown-type))[[X]]', blockId: 'block-1' })
  ...
  it('((depends-on))[[X]] 的字符偏移正确写入 data 属性', () => {
    const html = renderContentToHtml({ segments: [], content: '((depends-on))[[X]]', blockId: 'block-1' })
  ...
  it('普通 [[X]] 不被识别为 typed link', () => {
    const html = renderContentToHtml({ segments: [], content: 'See [[X]] plain', blockId: 'block-1' })
  ...
  it('((required-by))[[First]] 的 style 属性不被 #tag 正则误匹配', () => {
    const html = renderContentToHtml({ segments: [], content: '((required-by))[[First]]', blockId: 'block-1' })
  ...
  it('((depends-on))[[A]] 的 #f5222d 颜色值不被 #tag 正则误匹配', () => {
    const html = renderContentToHtml({ segments: [], content: '((depends-on))[[A]]', blockId: 'block-1' })
    expect(html).toMatch(/style="--rel-color:#f5222d"/)
    expect(html).not.toContain('data-page="f5222d"')
  })

  it('((is-a))[[A]] 的 #1890ff 颜色值不被 #tag 正则误匹配', () => {
    const html = renderContentToHtml({ segments: [], content: '((is-a))[[A]]', blockId: 'block-1' })
    expect(html).toMatch(/style="--rel-color:#1890ff"/)
    expect(html).not.toContain('data-page="1890ff"')
  })

  it('typed link 与 #tag 共存时两者都正确渲染', () => {
    const html = renderContentToHtml({ segments: [], content: 'see #myproject and ((related))[[A]]', blockId: 'block-1' })
...
describe('dateRef 渲染', () => {
  it('@2026-07-15 📅 渲染为 date-ref schedule span（含 data-raw）', () => {
    const html = renderContentToHtml({ segments: [], content: '任务 @2026-07-15 📅', blockId: 'block-1' })
  ...
  it('@2026-07-15T14:00 ⏰|weekly 渲染为 date-ref deadline span（含 data-raw）', () => {
    const html = renderContentToHtml({ segments: [], content: '@2026-07-15T14:00 ⏰|weekly', blockId: 'block-1' })
  ...
  it('无 recurrence 时 data-recurrence="none"', () => {
    const html = renderContentToHtml({ segments: [], content: '@2026-07-15 📅' })
    expect(html).toMatch(/data-recurrence="none"/)
  })

  it('daily/monthly/yearly 重复规则正确', () => {
    const html = renderContentToHtml({ segments: [], content: '@2026-07-15 📅|daily @2026-07-15 ⏰|yearly' })
  ...
  it('显示文本为原始语法（与编辑态一致）', () => {
    const html = renderContentToHtml({ segments: [], content: '@2026-07-15T14:00 📅' })
    expect(html).toContain('@2026-07-15T14:00 📅')
  })

  it('多个 dateRef 各自渲染', () => {
    const html = renderContentToHtml({ segments: [], content: '@2026-07-15 📅 和 @2026-07-16 ⏰' })
    const matches = html.match(/class="date-ref (schedule|deadline)/g)
    expect(matches).toHaveLength(2)
  })

  it('dateRef 与 wiki link 混合时两者都渲染', () => {
    const html = renderContentToHtml({ segments: [], content: '@2026-07-15 📅 参见 [[项目A]]' })
    expect(html).toMatch(/class="date-ref schedule"/)
    expect(html).toContain('data-page="项目A"')
  })

  it('dateRef 与 #tag 混合时两者都渲染', () => {
    const html = renderContentToHtml({ segments: [], content: '@2026-07-15 ⏰ #重要任务' })
    expect(html).toMatch(/class="date-ref deadline/)
  ...
  it('未来 deadline 不标记 overdue', () => {
    ...
    const html = renderContentToHtml({ segments: [], content: '@2026-07-16T14:00 ⏰' })
    expect(html).toMatch(/class="date-ref deadline"/)
  ...
  it('带 leadMinutes 的 dateRef 输出 data-lead-minutes 属性', () => {
    const html = renderContentToHtml({ segments: [], content: '@2026-07-15T14:00 📅|weekly|15' })
