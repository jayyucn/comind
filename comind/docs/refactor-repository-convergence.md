...
DateRef → Block → Page → Link → Property → RelationshipType → Template → Search → BlockVersion → Notification → SavedFilter → ScreenView → NotificationConfig. When the last entity lands, delete `SQLiteTransactionAdapter` entirely.
