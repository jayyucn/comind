...
- 关联代码：
  ...
  - `src/components/query/PageFieldRefPicker.vue`（跨记录：选页 + 选同类型字段）
  - `src/components/query/ConditionRow.vue` / `ConditionGroup.vue` / `FilterBuilder.vue`（props 透传 `availablePages`）
  - `src/composables/usePageQueryEngine.ts` / `useBlockQueryEngine.ts`（透传 `context`）
  - `src/components/PagesLibrary/PagesLibrary.vue`（`getById` 上下文 + `availablePages` 注入）
...
export type ConditionValue =
  ...
  | { kind: 'pageField'; pageId: string; field: string } // 跨记录页面字段引用
```
...
- 固定值输入框内有一个 **`+` 按钮**，弹出分层菜单：
  ...
  - 「其他页面…」→ 打开 `PageFieldRefPicker`：先按标题搜索并选 Page，再平列该 Page 上**同类型**字段（`{ kind:'pageField', pageId, field }`）。
...
- 同记录 `field` 引用**免费**（只需 registry 列举同类型字段）；跨记录 `pageField` 引用**需要 `QueryContext.getById`**（D3）。
...
- `resolveTarget` 解析三种 `kind`：
  ...
  - `pageField`：`context.getById('page', cv.pageId)` 取目标 Page → `registry.get('page', cv.field).get(targetItem)`。目标取不到或字段为空 → 返回 `undefined` → 比较类操作符遇空 → **非匹配**。

**桥接层接线**（`usePageQueryEngine` / `useBlockQueryEngine`）：
- 两引擎的 `filterSort*` / `group*` / `run*` 函数新增可选 `context?: QueryContext` 参数，透传给 `evaluate`。
- `PagesLibrary.vue` 提供 `queryContext = { getById: (et, id) => et === PAGE_ENTITY ? pageStore.getPage(id) : undefined }`（用**全量** store，不受列表搜索过滤影响），并注入 `availablePages = pageStore.pages.map(p => ({ id, title }))` 给 `FilterBuilder` 的「其他页面」入口。
- 不传 `context` 时（`TaskHub` 等尚未接入跨记录的调用方）`pageField` 引用一律非匹配——这是**安全的默认行为**，不会误判。
...
- UI 层：新增 `PageFieldRefPicker.vue`；重写 `ValueEditor.vue`（分段 + 引用菜单 + 芯片）；`ConditionRow` / `ConditionGroup` / `FilterBuilder` 透传 `availablePages`。
- 桥接层：`usePageQueryEngine` / `useBlockQueryEngine` 透传 `context`；`PagesLibrary.vue` 注入 `getById` + `availablePages`。
- 测试：新增 `evaluate-refs.test.ts`（field / pageField 解析，含「无 context」「目标缺失」「嵌套组」）；`FilterBuilder.test.ts` 新增字段引用 / 跨记录序列化 UI 测试；补齐两个 `use*QueryRegistry` 测试的 `cond` 助手迁移。
...
| 术语 | 含义 |
| --- | --- |
| **ConditionValue** | 条件值的判别联合：`literal`（字面量）/ `field`（同记录字段引用）/ `pageField`（跨记录页面字段引用）。`Condition.value` 的类型。 |
| **literal** | 字面量值。与旧版 `value` 等价，纯 JSON 直接存。 |
| **field（字段引用）** | 同记录字段引用：`{ kind:'field', field }`。求值时取当前记录上另一字段的值，实现字段间比较（如「字数 > 子页面数」）。 |
| **pageField（页面字段引用）** | 跨记录页面字段引用：`{ kind:'pageField', pageId, field }`。求值时经 `getById` 取目标 Page 再取其字段值。 |
| **QueryContext.getById** | 按 `entityType + id` 取实体对象的可选能力；跨记录引用解析的唯一把手。不提供时 `pageField` 一律非匹配。 |
| **availablePages** | 跨记录引用可选页面列表（`{ id, title }`），由 `PagesLibrary` 从页面 store 注入 `FilterBuilder` 的「其他页面…」入口。 |
