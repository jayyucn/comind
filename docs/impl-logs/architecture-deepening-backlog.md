...
- **Problem**：三对近平行模块；ADR-0029/0010 的 leverage 被逐屏复制的编排代码稀释。
...
- **依赖**：候选 6（ValueEditor 解耦）✅ 已铺垫；ADR-0028 注入点验证不变 ✅。
- **grill 共识速览**（决策细节见 ADR-0022，Q1–Q8 共 8 项）
  ...
  - **不变项（验证）**：`crossRecordSources` 注入点（ADR-0028 固定，PagesLibrary 翻译注入）。
- **🔸 待 /implement / 开放 follow-up**
...
- **依赖**：候选 7（`FieldDescriptor` / 值编辑器合一）🟡 已 grill 待 /implement；ADR-0028 缺口重开。
- **🔸 待 grill / 开放决策**
