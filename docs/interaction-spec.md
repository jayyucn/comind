...
**用途：** 层级线用于直观展示 Block 的嵌套层级关系，帮助用户识别父子结构。

**显示条件：**

- Block 拥有子 Block（`children.length > 0`）
- Block 处于展开态（`collapsed === false`）

**垂直高度规则：**

- 层级线从当前 Block **下方**开始延伸至最后一个子孙 Block 的底部
- 高度 = 最后一个子孙 Block 底部 - 当前 Block 底部

**水平位置：**

- `left: indentDepth * 24 + 9px`
- `indentDepth` 为当前 Block 的嵌套深度

**样式：**

- 宽度：1px
- 颜色：accent 颜色，透明度 0.15
- 样式：实线
- 过渡动画：200ms ease-out
...
| 场景                          | 行为                                         | 状态 |
| --------------------------- | ------------------------------------------ | -- |
| Block 无子 Block               | 不显示层级线                                     | ✅  |
| Block 处于折叠态（collapsed=true） | 不显示层级线                                     | ✅  |
| 嵌套层级极深（> 10 层）             | 层级线正常显示，视觉上可能重叠或过密                 | ✅  |
| 展开/折叠切换时                   | 层级线高度随子 Block 高度变化而变化（过渡动画 200ms）    | ✅  |
| 缩进/反缩进操作后                  | 层级线水平位置随 `left` 值变化而调整                | ✅  |
