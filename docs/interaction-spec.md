...
| 点击折叠态（collapsed=true）的父 Block 内容区 | 进入 `edit` 态，父 Block 展开，子 Block 不自动进入编辑 | ✅ 已实现（折叠切换与编辑态独立） |
...
| 场景 | 行为 |
|------|------|
| 无子 Block 时点击折叠图标 | 不做任何操作（图标本应隐藏，但代码应防御） |
| 折叠/展开动画进行中再次点击 | 立即切换到目标状态（无需等待动画完成） |
| 折叠态 Block 被拖拽 | 拖拽整棵子树（父 + 所有可见子 Block） |
