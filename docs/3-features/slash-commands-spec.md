...
| `/tag` / `/标签` | 插入 Page 链接 | 插入 `#` 字符（渲染为 Page 链接） |
...
**键盘选择稳定性**：
- `selectedIndex` 仅在查询文本实际变化时重置为0
- `flatCommands` 从 `groupedCommands` 派生，保证渲染顺序与选择顺序一致
- 避免 ProseMirror 编辑器更新导致选中状态意外重置
...
模板命令提供快速插入预定义结构化内容的能力。详见[模板系统规格](./template-system-spec.md)。

| 命令类型 | 描述 | 来源 |
|----------|------|------|
| 内置模板 | 系统预定义的10个模板 | `builtin` |
| 用户模板 | 用户自定义创建的模板 | `user` |

**内置模板分类**：
- 🤔 思维模型：二阶思维、5WHY分析、MECE拆解、第一性原理、预先验尸
- 📝 工作：会议记录、决策记录、阅读笔记
- 📋 回顾：每周复盘
- 🌅 日志：今日记录

**模板命令触发**：
- 输入 `/template` 或 `/tpl` 可过滤显示所有模板
- 输入模板名称（如 `/二阶思维`）可直接匹配对应模板
- 支持模板别名搜索

**模板命令执行流程**：
1. 清除斜杠命令文本
2. 加载和合并内置+用户模板
3. 构建模板上下文（日期、时间、页面标题等）
4. 渲染模板为 BlockDraft 列表
5. 按 pos 倒序插入新块（保证顺序正确）
6. 定位光标到第一个含 `{{cursor}}` 标记的块
...
- [模板系统规格](./template-system-spec.md)
