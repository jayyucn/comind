# ADR-0033: TableView 列宽拖拽缩放（表头手柄 + 持久化至 TableConfig）

- Status: accepted
- Date: 2026-08-23
- Supersedes: —
- Related: ADR-0005 (View/ViewQuery 模型), ADR-0006 (LayoutConfig schema), ADR-0027 (TableView 与 Task 解耦), ADR-0031 (TableView 字段管理面板)
...
### 2. 持久化归属（grill Q2）
- `TableView` 仍是**纯渲染器 / 哑发射器**：拖拽结束 `emit('columnResize', changes)`，其中 `changes: { key, width }[]` 一次性带回本列**与**相邻下一列的最终像素宽，**不**直接写 store、不表达业务意图（延续 ADR-0027/0011 解耦契约）。
- 宽度落库走与「列显隐 / 列排序」**完全相同**的通道：`QueryPageFrame` 监听 `@column-resize`，把 `changes` 汇总成 `Map<key,width>` 后 `store.patchActiveTabConfig((cfg) => ({ ...cfg, columns: cfg.columns.map((c) => widths.has(c.key) ? { ...c, width: widths.get(c.key) } : c) }))`。这是 ADR-0031 已确立的写回模式，缩放只是新增一种列变更意图。
...
- **正面**：通用表格首次获得「表头拖拽缩放列宽」能力，且与 ADR-0031 的「列显隐/排序」共用一套 patch 通道，架构一致；`TableView` 仍零业务耦合、可复用于任意 Screen（Block / Page 各自注入 `fields`）；完全复用现有 `TableConfig.columns` 持久化，不新增数据结构。
