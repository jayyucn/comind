# ADR-0028: 三个视图全部通用化并迁移至 components/views

- Status: accepted
- Date: 2026-08-17
- Extends: ADR-0005 (View 模型), ADR-0006 (LayoutConfig), ADR-0027 (TableView 解耦)
- Related: ADR-0002 (字段注册表解耦)

## Context

ADR-0027 把 `TableView` 改写为实体无关、字段驱动的通用表，但当时 `BoardView`/`CalendarView` 仍是**任务专属硬编码**（写死 `STATUS_OPTIONS`/`PRIORITY_COLORS`、`BlockCard` 强绑、`date_refs` 直读），且三个视图都驻在 `TaskHub/views/`。这与 ADR-0005「视图通用于任意实体」只落地了 1/3，造成两个真问题：
...
- `useBlockQueryRegistry` 新增 `schedule` 内置 date 字段（配合 `CalendarConfig.dateRefKind='schedule'`）；`deadline` 字段 `get` 修复为 `find(deadline) ?? find(schedule)`（见 ADR-0027 D1）。
...
- **需跟进**：
  - D3/D4（年份格式、完成进度头）仍为已知 UX 技术债（ADR-0027）。
