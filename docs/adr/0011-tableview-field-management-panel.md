# ADR-0011: TableView 字段管理面板（per-tab 显示/隐藏 + 全局增/删）

- Status: accepted
- Date: 2026-08-22
- Supersedes: —
- Related: ADR-0005 (View/ViewQuery 模型), ADR-0006 (LayoutConfig schema), ADR-0007 (TableView 与 Task 解耦), ADR-0009 (Screen→Tab 两级 NamedView), ADR-0010 (TableView Custom Cell Renderer)

## Context

`TableView.vue`（`src/components/views/TableView.vue`，`generic="T"`）是实体无关的字段驱动表，按 `config.columns`（每个 Tab 持久化的 `TableConfig`）渲染列，列数据来自字段池 `props.fields: FieldDescriptor[]`。组件零业务耦合（ADR-0007）。
