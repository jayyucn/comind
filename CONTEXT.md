...
### Snapshot Context / Live Context (快照上下文 / 活上下文)
The two editability contexts of BlockModal: opened from a snapshot rendering, it renders the snapshot's data **read-only**; opened from block-level paths (Task Hub cards, bullet-to-open), it renders the live Block **fully editable** — regardless of the page's age. Editability is decided by context, never by page date. See ADR-0042. _Avoid_: frozen block, readonly-by-page.
...
### Block Selection (块选区)
A set of Block ids selected as whole units, toggled via Ctrl/Cmd+Click anywhere on the block row — including the property strip and the 8px band beneath it — except the bullet column. Powers block-level copy/paste/cut/delete (ADR-0025/0026). Distinct from **Text Range**. See ADR-0035 (D2/D6). _Avoid_: 多块选择, multi-select, 整块选择.

### Text Range (文本选区)
A contiguous text range spanning multiple Blocks, bounded by two char positions — each a `{ blockId, offset }` into that block's raw content — with everything between them in document order selected, like a word processor. Created by dragging across block content, or by Shift+Click (extends an existing range's head to the clicked position; with no existing range, starts one from the caret). Powers text copy and keyboard deletion (Backspace/Delete trim the two endpoints by character and merge them into one block, removing middle blocks entirely; endpoints of non-text types are left intact) and cut (Ctrl/Cmd+X = copy with the Ctrl+C semantics, then delete with the Backspace semantics). Distinct from **Block Selection**; the two are mutually exclusive. See ADR-0035 (D5/D8/D10). _Avoid_: 文字选择, selection range, 文本选择.
