...
/// 一次调用完成整条重排（优于逐条 `updateTab`）。调用方须保证 `ordered_ids` 覆盖该范围内全部子项；
/// 若缺漏，未列 id 保持其旧 `sort_order`，可能与新序产生间隙——由上层（store 计算完整顺序）保证。
/// 校验归属：任一 id 不属于该 `(entity, parent_id)` 即拒绝，防止越权写入其它层级。
#[cfg(not(target_arch = "wasm32"))]
